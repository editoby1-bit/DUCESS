...
const ROLE_DEFAULT_TOOLS = {
  ...
  function normalizeCodSubmissionRecord(row) {
    ...
    return {
      ...
      overdraw: row.overdraw != null ? Number(row.overdraw) : Math.max(0, -(Number(row.remainingBalance ?? row.remaining_balance ?? 0))),
      note: row.note || '',
...
async function submitDebtRepayment(_payload) {
  ...
  function createLocalAdapter(options = {}) {
    ...
    return {
      ...
      appState,
      audit: {
        listAuditLog: async () => ({ ok: true, data: [] }),
        insertAuditLogEntry: async () => ({ ok: true, data: null }),
      },
      __meta: {
        adapter: 'local',
        phase: '3B.3',
...
return defaultResult.ok(normalizeApprovalRecord(data));
  ...
  requestRow.payload = {
    ...
    async function fetchApprovedCustomerTransactionsForStaffDate(staffId, businessDate) {
      let query = client.from(customerTransactionsTable).select(customerTransactionsSelect);
